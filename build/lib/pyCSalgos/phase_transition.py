...
class PhaseTransition(with_metaclass(ABCMeta, object)):
    ...
    def savedata(self, basename=None):
        ...
        mdict = {'signaldim': self.signaldim, 'dictdim': self.dictdim, 'numdata': self.numdata,
                 'deltas': self.deltas, 'rhos': self.rhos, 
                 'snr_db_sparse': self.snr_db_sparse, 'snr_db_signal': self.snr_db_signal, 'snr_db_meas': self.snr_db_meas,
                 'solverNames': self.solverNames, 'ERCsolverNames': self.ERCsolverNames,
                 'err': self.err, 'ERCsuccess': self.ERCsuccess, 'gamma': self.gamma, 'support': self.support, 'simData': self.simData,
    ...
    def loaddata(self, matfilename=None, picklefilename1=None, picklefilename2=None):
        ...
        if matfilename is not None or picklefilename2 is not None:
            ...
            self.snr_db_sparse = mdict['snr_db_sparse']
            self.snr_db_signal = mdict['snr_db_signal']
            self.snr_db_meas   = mdict['snr_db_meas']
            self.solverNames = mdict['solverNames'].copy()
            self.ERCsolverNames = mdict['ERCsolverNames'].copy()
            if mdict['err'] is not None:
                self.err = mdict['err'].copy()
            if mdict['ERCsuccess'] is not None:
                self.ERCsuccess = mdict['ERCsuccess'].copy()
            if mdict['gamma'] is not None:
                self.gamma = mdict['gamma'].copy()
            if mdict['support'] is not None:
                self.support = mdict['support'].copy()   # This is a list
    ...
    def run(self, solve=True, check=False, processes=None, random_state=None):
        ...
        if solve or check:
            ...
            if processes is not 1:
                ...
                results = list(map(run_synthesis_delta_rho, enumerate(task_parameters)))
    ...
    def plot_suppport_recovered(self, basename=None, saveexts=[], showtitle=False, legend=[], rhomax=1):
        ...
        for idelta, _ in enumerate(self.deltas):
            for irho, _ in enumerate(self.rhos):
                realsupp = self.simData[idelta][irho]['realsupport']
